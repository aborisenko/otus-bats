...
object Show {
  ...
  implicit def listShow[A](implicit ev: Show[A]): Show[List[A]] = new Show[List[A]] {
    override def show(a: List[A]): String = a.foldLeft("")( (acc,a) => acc ++ ev.show(a))
  }
   ...
   */
  def mkString_[A: Show](list: List[A], begin: String, end: String, separator: String): String = list.headOption.map { head =>
    list.tail.foldLeft(begin ++ head.show)((acc, a) => acc ++ separator ++ a.show) ++ end
  }.getOrElse("")
